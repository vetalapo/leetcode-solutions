 ...
 *
 * R eturn the smallest index i such that s[*i] == s[n - i - 1].
 *
 ...
 *     No smaller index satisfies the condition,
 *     so the answer is 1.
 *
 * Example 2:
 *   Input: s = "abc"
 *   Output: 1
 *   Explanation:
 *     ​​​​​​​At index i = 1, the two compared positions coincide,
 *     so both characters are 'b'.
 *     No smaller index satisfies the condition,
 *     so the answer is 1.
 *
 * Example 3:
 ...
 *     Therefore, no valid index exists,
 *     so the answer is -1.
 *
 * Constraints:
 *   • 1 <= n == s.length <= 100
