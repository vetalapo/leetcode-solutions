 ...
 *   Can you try the combination of every two houses?
 *
 * Hint 2:
 *   Greedily, the maximum distance will come from either the pair of the leftmost house
 *   and possibly some house on the right with a different color,
 *   or the pair of the rightmost house
 *   and possibly some house on the left with a different color.
 **
   ...
   public int MaxDistance( int[] colors )
    {
        ...
        {
            if ( colors[i] != colors[right] || colors[0] != colors[right - i] )
            {
                return right - i;
            }
        }
