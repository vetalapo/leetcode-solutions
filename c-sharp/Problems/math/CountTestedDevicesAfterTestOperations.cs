 ...
 * that will be tested after performing the test operations in order.
 *
 * Example 1:
 ...
 *       - At device 0,
 *         batteryPercentages[0] > 0,
 *         so there is now 1 tested device,
 *         and batteryPercentages becomes [1,0,1,0,2].
 *       - At device 1,
 *         batteryPercentages[1] == 0,
 *         so we move to the next device without testing.
 *       - At device 2,
 *         batteryPercentages[2] > 0,
 *         so there are now 2 tested devices,
 *         and batteryPercentages becomes [1,0,1,0,1].
 *       - At device 3,
 *         batteryPercentages[3] == 0,
 *         so we move to the next device without testing.
 *       - At device 4,
 *         batteryPercentages[4] > 0,
 *         so there are now 3 tested devices,
 ...
 *
 * Hint 1:
 *   One solution is simulating the operations
 *   as explained in the problem statement,
 *   and it works in O(n2) time.
 *
 * Hint 2:
 *   While going through the devices,
 *   you can maintain the number of previously tested devices,
 *   and the current device can be tested
 *   if batteryPercentages[i] is greater
 *   than the number of tested devices.
 **
...
public class CountTestedDevicesAfterTestOperations
{
    public int CountTestedDevices( int[] batteryPercentages )
    {
        ...
        foreach ( int percentage in batteryPercentages )
        {
            if ( percentage > count )
            {
                count++;
