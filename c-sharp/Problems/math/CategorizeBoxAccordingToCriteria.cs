...
public class CategorizeBoxAccordingToCriteria
{
    ...
    {
        bool isBulky = length >= 10000 ||
            width >= 10000 ||
            height >= 10000 ||
            GetVolume( length, width, height ) >= 1000000000;
    ...
    private long GetVolume( int length, int width, int height )
    {
        return length * width * (long)height;
    }
}
