 ...
 * which represents a sequence of events
 * where a child pushes a series of buttons on a keyboard.
 *
 * Each events[i] = [index^i, time^i] indicates
 * that the button at index index^i was pressed at time time^i.
 *
 * The array is sorted in increasing order of time.
 *
 * The time taken to press a button
 * is the difference in time between consecutive button presses.
 * The time for the first button is simply the time at which it was pressed.
 *
 * Return the index of the button
 * that took the longest time to push.
 ...
 *     - Button with index 1 is pressed again at time 15,
 *     so it took 15 - 9 = 6 units of time.
 *
 * Example 2:
 ...
 *     - Button with index 1 is pressed at time 7,
 *     so it took 7 - 5 = 2 units of time.
 *
 * Constraints:
 *   • 1 <= events.length <= 1000
 *   • events[i] == [index^i, time^i]
 *   • 1 <= index^i, timei <= 10^5
...
public class ButtonWithLongestPushTime
{
    public int ButtonWithLongestTime( int[][] events )
    {
        ...
        for ( int i = 1; i < events.Length; i++ )
        {
            ...
            {
                maxTime = diff;
                maxResult = events[i][0];
            }
            else if ( diff == maxTime )
            {
                maxResult = Math.Min( maxResult, events[i][0] );
            }
        }
