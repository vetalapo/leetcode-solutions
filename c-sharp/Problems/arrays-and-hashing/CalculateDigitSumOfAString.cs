 ...
 *   1. Divide s into consecutive groups of size k such
 *      that the first k characters are in the first group,
 *      the next k characters are in the second group, and so on.
 *      Note that the size of the last group can be smaller than k.
 *   2. Replace each group of s with a string
 *      representing the sum of all its digits.
 *      For example,
 *      "346" is replaced with "13" because 3 + 4 + 6 = 13.
 *   3. Merge consecutive groups together
 ...
 *     Now, s.length <= k,
 *     so we return "135" as the answer.
 *
 * Example 2:
