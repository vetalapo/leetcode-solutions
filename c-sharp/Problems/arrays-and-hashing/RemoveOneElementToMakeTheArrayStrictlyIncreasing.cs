 ...
 * return true if it can be made strictly increasing
 ...
 *
 * Hint 2:
 *   If the array becomes sorted return true,
 *   otherwise revert to the original array and try different index.
 **
...
public class RemoveOneElementToMakeTheArrayStrictlyIncreasing
{
    public bool CanBeIncreasing( int[] nums )
    {
        ...
        for ( int i = 1; i < nums.Length && dropCount < 2; i++ )
        {
            if ( nums[i - 1] >= nums[i] )
            {
                dropCount++;
