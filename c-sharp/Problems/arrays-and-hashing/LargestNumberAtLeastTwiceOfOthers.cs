 ...
 *   Output: -1
 *   Explanation:
 *     4 is less than twice the value of 3,
 *     so we return -1.
 *
 * Constraints:
 *   • 2 <= nums.length <= 50
 ...
 *   If we find some `x != m` with `m < 2*x`,
 *   we should return `-1`.
 *   Otherwise,
 *   we should return `maxIndex`.
 **
...
public class LargestNumberAtLeastTwiceOfOthers
{
    public int DominantIndex( int[] nums )
    {
        ...
        {
            if ( maxIndex != i && nums[i] > nums[maxIndex] / 2 )
            {
                return -1;
            }
        }
