 ...
 **
 * There is a malfunctioning keyboard
 * where some letter keys do not work.
 * All other keys on the keyboard work properly.
 *
 * Given a string text of words separated by a single space
 * (no leading or trailing spaces)
 * and a string brokenLetters of all distinct letter keys
 * that are broken,
...
public class MaximumNumberOfWordsYouCanType
{
    public int CanBeTypedWords( string text, string brokenLetters )
    {
        ...
        while ( i <= text.Length )
        {
            ...
            else if ( isValid && i < text.Length && brokenMap[text[i] - 'a'] )
            {
                isValid = false;
            }
