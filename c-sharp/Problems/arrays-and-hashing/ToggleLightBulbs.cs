 ...
 * All of them are switched off initially.
 *
 * For each element bulbs[i] in the array bulbs:
 *   • If the bulbs[i]th light bulb is currently off, switch it on.
 *   • Otherwise, switch it off.
 *
 * Return the list of integers denoting the light bulbs
 * that are on in the end, sorted in ascending order.
...
public class ToggleLightBulbsSolution
{
    public IList<int> ToggleLightBulbs( IList<int> bulbs )
    {
        bool[] bulbMap = new bool[101];

        foreach ( int bulb in bulbs )
        {
            bulbMap[bulb] = !bulbMap[bulb];
        }

        List<int> result = new();

        for ( int i = 1; i < bulbMap.Length; i++ )
        {
            if ( bulbMap[i] )
            {
                result.Add( i );
