 ...
 * In one operation,
 * select any index i such that 0 < i < words.length
 * and words[i - 1] and words[i] are anagrams,
 * and delete words[i] from words.
 * Keep performing this operation as long
 * as you can select an index that satisfies the conditions.
 *
 ...
 *       we choose index 2 and delete words[2].
 *       Now words = ["abba","cd"].
 *     We can no longer perform any operations,
 *     so ["abba","cd"] is the final answer.
 *
 * Example 2:
 *   Input: words = ["a","b","c","d","e"]
 *   Output: ["a","b","c","d","e"]
 *   Explanation:
 *     No two adjacent strings in words are anagrams of each other,
 ...
 *   try to find all the strings in words
 *   which will not be present in the final answer.
 *
 * Hint 2:
 *   For every index i,
 *   find the largest index j < i such that words[j]
 *   will be present in the final answer.
 *
 * Hint 3:
 *   Check if words[i] and words[j] are anagrams.
 *   If they are,
 *   then it can be confirmed that words[i]
 *   will not be present in the final answer.
 **
