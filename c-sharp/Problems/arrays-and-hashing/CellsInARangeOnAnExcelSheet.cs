 ...
 *     It is rep*resented by alphabetical letters.
 ...
 * where <col1> represents the column c1,
 * <row1> represents the row r1,
 * <col2> represents the column c2,
 * and <row2> represents the row r2,
 * such that r1 <= r2 and c1 <= c2.
 *
 * Return the list of cells (x, y) such
 * that r1 <= x <= r2 and c1 <= y <= c2.
...
public class CellsInARangeOnAnExcelSheet
{
    public IList<string> CellsInRange( string s )
    {
        ...
        for ( char col = s[0]; col <= s[3]; col++ )
        {
            ...
            {
                result.Add( col.ToString() + row.ToString() );
            }
        }
