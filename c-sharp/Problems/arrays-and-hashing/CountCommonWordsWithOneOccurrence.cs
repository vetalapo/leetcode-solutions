 ...
 *     - "as" appears once in words1,
 *       but does not appear in words2.
 *       We do not count this string.
 *     Thus,
 *     there are 2 strings that appear exactly once
 ...
 *   Output: 1
 *   Explanation:
 *     The only string that appears exactly once
 *     in each of the two arrays is "ab".
 *
 * Constraints:
...
public class CountCommonWordsWithOneOccurrence
{
    public int CountWords( string[] words1, string[] words2 )
    {
        ...
        {
            if ( countMap.TryGetValue( word, out int value ) && value < 2 )
            {
                countMap[word]--;
            }
        }
        ...
        foreach ( int value in countMap.Values )
        {
            if ( value == 0 )
            {
                count++;
